...
class ControllerPaymentTwoCheckoutAPI extends Controller {
    protected function index() {
        ...
        $this->data['twocheckoutAPI_sid'] = $this->config->get('twocheckoutAPI_account');
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/twocheckoutAPI.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/twocheckoutAPI.tpl';
        } else {
            $this->template = 'default/template/payment/twocheckoutAPI.tpl';
        }
    ...
    public function send() {
        ...
        $params = array(
            "sellerId" => $this->config->get('twocheckoutAPI_account'),
        ...
        try {
            if ($this->config->get('twocheckoutAPI_test')) {
                TwocheckoutApi::setCredentials($this->config->get('twocheckoutAPI_account'), $this->config->get('twocheckoutAPI_private_key'), 'sandbox');
            } else {
                TwocheckoutApi::setCredentials($this->config->get('twocheckoutAPI_account'), $this->config->get('twocheckoutAPI_private_key'));
            }
            $charge = Twocheckout_Charge::auth($params);

            $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('config_order_status_id'));
            $message = '2Checkout Order: ' . $charge['response']['orderNumber'];
            $this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('twocheckoutAPI_order_status_id'), $message, false);
